...
class Model_MF(nn.Module):
    def __init__(self, equation, dim_h):
        ...
        self.linear1 = nn.Linear(2*equation.dim_x + 1, dim_h)
    ...
    def forward(self, N, n, x):
        ...
        Ex = torch.mean(x, dim=0, keepdim=True).repeat(x.size(0),1)
        ...
        inpt = torch.cat((x_nor, Ex, torch.ones(x.size()[0], 1, device=device) * delta_t * n), 1)
        yz = phi(inpt)
...
class BSDEsolver():
    ...
    def train(self, batch_size, N, n, itr, path, multiplyer):
        ...
        if n != N-2:
            mod2 = Model_MF(self.equation, self.dim_h).to(device)
    ...
    def train_whole(self, batch_size, N, path, itr, multiplyer):
        ...
        for n in range(N-2,-1,-1):
            ...
            if n==N-2:
                coeff = 1
            print("time "+ str(n))
            mod = Model_MF(self.equation, self.dim_h).to(device)
